...
import club.hanfei.model.feed.RSSCategory;
import club.hanfei.model.feed.RSSChannel;
import club.hanfei.model.feed.RSSItem;
...
public class FeedProcessor {
    ...
    public void genRecentRSS(final RequestContext context) {
        ...
        try {
            ...
            final JSONObject result = articleQueryService.getRecentArticles(UserExt.USER_AVATAR_VIEW_MODE_C_STATIC, 0, 1, Hanfei
                .getInt("indexArticlesCnt"));
            ...
            channel.setGenerator("Symphony v" + HanfeiServletListener.VERSION + ", https://sym.b3log.org");
    ...
    public void genDomainRSS(final RequestContext context) {
        ...
        try {
            ...
            final JSONObject result = articleQueryService.getDomainArticles(UserExt.USER_AVATAR_VIEW_MODE_C_STATIC, domainId, 1, Hanfei
                .getInt("indexArticlesCnt"));
            final List<JSONObject> articles = (List<JSONObject>) result.get(Article.ARTICLES);
            for (int i = 0; i < articles.size(); i++) {
                RSSItem item = getItem(articles, i);
