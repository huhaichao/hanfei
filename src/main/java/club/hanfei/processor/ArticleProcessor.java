...
import club.hanfei.processor.advice.validate.ArticleAddValidation;
import club.hanfei.processor.advice.validate.ArticleUpdateValidation;
...
import club.hanfei.service.LivenessMgmtService;
import club.hanfei.service.ReferralMgmtService;
import club.hanfei.service.RevisionQueryService;
import club.hanfei.service.RewardQueryService;
...
import club.hanfei.util.MP3Players;
import club.hanfei.util.Markdowns;
import club.hanfei.util.StatusCodes;
import club.hanfei.util.VideoPlayers;
...
public class ArticleProcessor {
    ...
    public void getArticleImage(final RequestContext context) {
        ...
        for (int i = 0; i < articleContent.length(); i++) {
            ...
            final JSONObject chRecord = Character.getCharacter(ch, characters);
            if (null == chRecord) {
                images.add(Character.createImage(ch));

                continue;
            }

            final String imgData = chRecord.optString(Character.CHARACTER_IMG);
...
//        }
    ...
    public void showArticle(final RequestContext context) {
        ...
        stickConfirmLabel = stickConfirmLabel.replace("{point}", Hanfei.get("pointStickArticle"));
        dataModel.put("stickConfirmLabel", stickConfirmLabel);
        dataModel.put("pointThankArticle", Hanfei.get("pointThankArticle"));

        int pageNum = Paginator.getPage(request);
        final int pageSize = Hanfei.getInt("articleCommentsPageSize");
        final int windowSize = Hanfei.getInt("articleCommentsWindowSize");
        ...
        if (!niceComments.isEmpty()) {
            ...
            for (final JSONObject comment : niceComments) {
                ...
                thankTemplate = thankTemplate.replace("{point}", String.valueOf(Hanfei.getInt("pointThankComment")))
        ...
        try {
            ...
            for (final JSONObject comment : articleComments) {
                ...
                final String thankStr = thankTemplate.replace("{point}", String.valueOf(Hanfei.getInt("pointThankComment")))
                        .replace("{user}", comment.optJSONObject(Comment.COMMENT_T_COMMENTER).optString(User.USER_NAME));
        ...
        if (StringUtils.isNotBlank(Hanfei.get("ipfs.dir"))) {
