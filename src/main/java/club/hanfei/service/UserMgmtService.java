...
import club.hanfei.util.Gravatars;
...
public class UserMgmtService {
    ...
    public synchronized String addUser(final JSONObject requestJSONObject) throws ServiceException {
        ...
        try {
            ...
            user.put(UserExt.USER_TAGS, "");
            user.put(UserExt.USER_SKIN, Hanfei.get("skinDirName"));
            user.put(UserExt.USER_MOBILE_SKIN, Hanfei.get("mobileSkinDirName"));
            user.put(UserExt.USER_COUNTRY, "");
            user.put(UserExt.USER_PROVINCE, "");
            user.put(UserExt.USER_CITY, "");
            ...
            user.put(UserExt.USER_LIST_PAGE_SIZE, Hanfei.getInt("indexArticlesCnt"));
            ...
            if (toUpdate) {
                ...
                if (!AvatarQueryService.DEFAULT_AVATAR_URL.equals(avatarURL)) { // generate/upload avatar succ
                    if (Hanfei.getBoolean("qiniu.enabled")) {
                        user.put(UserExt.USER_AVATAR_URL, Hanfei.get("qiniu.domain") + "/avatar/" + ret + "?"
                                + new Date().getTime());
                    } else {
                ...
                if (AvatarQueryService.DEFAULT_AVATAR_URL.equals(specifiedAvatar)) {
                    ...
                    try {
                        ...
                        if (Hanfei.getBoolean("qiniu.enabled")) {
                            ...
                            uploadManager.put(avatarData, "avatar/" + ret, auth.uploadToken(Hanfei.get("qiniu.bucket")),
                                    null, "image/jpeg", false);
                            user.put(UserExt.USER_AVATAR_URL, Hanfei.get("qiniu.domain") + "/avatar/" + ret + "?" + new Date().getTime());
                        } else {
