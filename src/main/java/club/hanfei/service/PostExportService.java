...
public class PostExportService {
    ...
    public String exportPosts(final String userId) {
        final int pointDataExport = Hanfei.getInt("pointDataExport");
        try {
        ...
        try {
            ...
            if (Hanfei.getBoolean("qiniu.enabled")) {
                ...
                uploadManager.put(zipData, fileKey, auth.uploadToken(Hanfei.get("qiniu.bucket")),
                        null, "application/zip", false);

                return Hanfei.get("qiniu.domain") + "/" + fileKey;
            } else {
                final String filePath = Hanfei.get("upload.dir") + fileKey;
